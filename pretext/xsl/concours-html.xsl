...
>
    ...
    <xsl:import href="./core/xml-to-json.xsl" />
    ...
    <xsl:import href="./core/pretext-runestone.xsl" />
    <xsl:import href="./core/pretext-runestone-fitb.xsl" />
    ...
    <xsl:import href="./core/pretext-view-source.xsl" />
    ...
    <xsl:variable name="html.css.dir" select="'/assets/css'" />
    <xsl:variable name="html.js.dir" select="concat($cdn-prefix, '_static/pretext/js')" />
    <xsl:variable name="html.jslib.dir" select="'/assets/js/pretext/lib/'" />
    ...
    <!-- fix right now.                                                     -->
    <xsl:variable name="b-has-sfrac"
        select="boolean($document-root//m[contains(text(),'sfrac')]|$document-root//md[contains(text(),'sfrac')]|$document-root//me[contains(text(),'sfrac')]|$document-root//mrow[contains(text(),'sfrac')])" />
    <xsl:variable name="b-has-geogebra"
        select="boolean($document-root//interactive[@platform='geogebra'])" />
    <xsl:variable name="b-has-mermaid"
        select="boolean($document-root//image[mermaid]|/image[mermaid])" />
    ...
    <xsl:variable name="b-has-prefigure-annotations"
        select="boolean($document-root//pf:prefigure/pf:diagram/pf:annotations)" />
    ...
    <xsl:key name="marker-key" match="ol|ol-marker" use="@marker" /> <!-- never used on node sets
    that contain mix of ol and ol-marker -->
    <xsl:variable name="ol-markers">
        <ol-markers>
            <xsl:apply-templates
                select="$document-root//ol[@marker and count(. | key('marker-key', @marker)[1]) = 1]"
                mode="ol-markers" />
        </ol-markers>
    </xsl:variable>
    ...
    <xsl:variable name="webwork-major-version"
        select="$document-root//webwork-reps[1]/@ww_major_version" />
    <xsl:variable name="webwork-minor-version"
        select="$document-root//webwork-reps[1]/@ww_minor_version" />
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when test="not($subtree-node)">
                        <xsl:message terminate="yes">PTX:ERROR: the @xml:id given as a subtree root
                            ("<xsl:value-of select="$subtree" />") does not specify any element.
                            (Check spelling?) Quitting...</xsl:message>
                    </xsl:when>
                    <xsl:when test="not($subtree-node[&STRUCTURAL-FILTER;])">
                        <xsl:message terminate="yes">PTX:ERROR: the element with the @xml:id given
                            as a subtree root ("<xsl:value-of select="$subtree" />") is not division
                            that can be chunked into HTML page(s). Quitting...</xsl:message>
                    </xsl:when>
                    <xsl:otherwise>
                        ...
                        <xsl:if test="$current-level > $chunk-level">
                            <xsl:message terminate="yes">PTX:ERROR: the element with @xml:id given
                                as a subtree root ("<xsl:value-of select="$subtree" />") is only a
                                partial HTML page at the current chunking level. Quitting...</xsl:message>
                        </xsl:if>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:template name="index-redirect-page">
        ...
        <xsl:if test="$chunk-level = ''">
            <xsl:message>PTX:BUG the $chunk-level variable has been left undefined&#xa;due to a
                change in a stylesheet that imports the HTML conversion&#xa;and the computation of
                an index page may fail spectacularly (infinite recursion?)"</xsl:message>
        </xsl:if>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:variable name="is-intermediate">
                        <xsl:apply-templates select="id($html-index-page-entered-ref)"
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:message>PTX:WARNING: the requested HTML index page cannot be
                                constructed since "<xsl:value-of
                                    select="$html-index-page-entered-ref" />" is not a complete web
                                page at the current chunking level (level <xsl:value-of
                                    select="$chunk-level" />). Defaults will be used instead</xsl:message>
                            <xsl:text />
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:otherwise>
        ...
        <xsl:variable name="html-index-page">
            <xsl:choose>
                ...
                <xsl:when test="$document-root/frontmatter and ($chunk-level &gt; 0)">
                    <xsl:apply-templates select="$document-root/frontmatter"
        ...
        <!-- <exsl:document href="index.html" method="html" indent="yes" encoding="UTF-8">
        <xsl:call-template name="jekyll-frontmatter"/>
        <html>
            <xsl:text>&#xa;</xsl:text>
        ...
        <li>
            <a href="{$url}" class="internal">
                ...
                <xsl:when test="self::chapter">
                    <xsl:apply-templates select="." mode="title-formated" />
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="exercises" mode="structural-division-inner-content">
        ...
        <xsl:choose>
            <xsl:when test="@time-limit and $b-is-groupwork">
                <xsl:message>PTX:ERROR: an &quot;exercises&quot; division cannot simultaneously be a
                    timed exam AND group work</xsl:message>
    ...
    <xsl:template match="*" mode="section-heading">
        ...
        <xsl:element name="{$html-heading}">
            <xsl:attribute name="class">
                <xsl:choose>
                    <xsl:when
                        test="(self::chapter or self::appendix or self::solutions/parent::backmatter) and ($numbering-maxlevel > 0)">
    ...
    <xsl:template match="exercises|solutions|glossary|references|worksheet|reading-questions"
    ...
    <xsl:template match="colophon-items">
        ...
        <!-- NB: keywords are included in a colophon for a book, but under the abstract of an
        article -->
        <xsl:apply-templates select="$bibinfo/keywords" />
        <xsl:apply-templates select="$bibinfo/support" />
    </xsl:template>
    ...
    <xsl:template
        match="introduction[parent::*[&STRUCTURAL-FILTER;]]|conclusion[parent::*[&STRUCTURAL-FILTER;]]">
        ...
        <xsl:variable name="is-child-of-structured">
            <xsl:choose>
                <xsl:when test="parent::*[&TRADITIONAL-DIVISION-FILTER;]">
                    <xsl:apply-templates select="parent::*[&TRADITIONAL-DIVISION-FILTER;]"
                        mode="is-structured-division" />
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="false()" />
                </xsl:otherwise>
        ...
        <xsl:variable name="xref-ids">
            <xsl:for-each select="$document-root//xref|$document-root//fragref">
                <xsl:choose>
                    ...
                    <xsl:when
                        test="@ref and (contains(normalize-space(@ref), ' ') or contains(@ref, ','))">
                        <xsl:variable name="clean-list"
                            select="concat(normalize-space(translate(@ref, ',', ' ')), ' ')" />
    ...
    <xsl:template match="*" mode="manufacture-knowl">
        ...
        <exsl:document href="{$knowl-file}" method="html" indent="yes" encoding="UTF-8"
    ...
    <xsl:template match="*" mode="html-heading">
        ...
        </xsl:text><xsl:apply-templates select="." mode="long-name"/> -->
        <!-- </xsl:message> -->
    ...
    <!-- REMARK-LIKE, COMPUTATION-LIKE, DEFINITION-LIKE, SOLUTION-LIKE, objectives (xref-content),
    outcomes (xref-content), EXAMPLE-LIKE, PROJECT-LIKE, OPENPROBLEM-LIKE, exercise (inline), task
    (xref-content), fn (xref-content), biblio/note (xref-content)-->
    ...
    <xsl:template match="*" mode="heading-full">
        ...
        <xsl:element name="{$hN}">
            ...
            <!--  -->
            <xsl:call-template name="before-styled" />
            ...
            <xsl:call-template name="after-styled" />
            <xsl:call-template name="space-styled" />
            <xsl:if test="title">
                <xsl:call-template name="slider-styled" />
            </xsl:if>
            <xsl:call-template name="collapser-styled" />
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="*" mode="assemb-title">
        <xsl:param name="heading-level" />
        <xsl:variable name="has-default-title">
            <xsl:apply-templates select="." mode="has-default-title" />
        </xsl:variable>
    ...
    <xsl:template name="slider-styled">
        <span class="slider button-box-custom">
            <xsl:text> </xsl:text>
        </span>
    </xsl:template>
    <xsl:template name="collapser-styled">
        <span class="collapser button-box-custom">
            <xsl:text> </xsl:text>
        </span>
    </xsl:template>
    <xsl:template name="before-styled">
        <span class="before">
            <xsl:text> </xsl:text>
        </span>
    </xsl:template>
    <xsl:template name="after-styled">
        <span class="after">
            <xsl:text> </xsl:text>
        </span>
    </xsl:template>
    ...
    <!-- 2023-09-23: content "born hidden" may be due to an election by a publisher ("theorem") or
    automatic as part of this conversion ("hint").  We once thought of these as "embedded" knowls
    since their content lived on the page, and in contrast to cross-reference knowls ("xref") whose
    content lives in an (external) file.  With recent changes we have migrated even further away from
    "embed" to "hidden" in nomenclature for these.  The comments below may not always reflect this. -->

    <!-- Generically, a "block" is a child of a "division."  See the schema for more precision.
    Blocks also have significant components.  An "example" is a block, and its "solution" is a
    significant component.  A "p" might be a block, but it could also be a significant component of
    an "example." -->

    <!-- Some blocks and components can be realized in a hidden fashion, as knowls whose content is
    embedded within the page.  This may be automatic ("hint" is always born hidden), elective
    ("theorem" is a good example), or banned (a "blockquote" is never hidden). -->

    <!-- All blocks, and many of their significant components, are available as targets of
    cross-references, implemented as knowls, but now the content resides in external files.  These
    files contain duplicates of blocks and their components (rather than originals), so need to be
    free of the unique identifiers that are used in the original versions. -->

    <!-- This suggests three modes for the initial production of a block or component, though some
    blocks may only be produced in two of the three modes: visible and original, hidden and
    original, a cross-reference knowl. -->
    ...
    <!-- The generic (not modal) template matches any element that is a block or a significant
    component of some other element that is a block or a component. -->

    <!-- Every such element is only output in one of two forms, and as few times as possible.  One
    form is the "original" and includes full identifying information, such as an HTML id attribute
    or a LaTeX label for rows of display mathematics.  The other form is a "duplicate", as an
    external file, for use by the knowl code to open and display.  As a duplicate of the orginal, it
    should be free of all identifying information and should recycle other duplicates as much as
    possible. -->

    <!-- An element arrives here in one of three situations, two as originals and one as a
    duplicate.  We describe those situations and what should happen. -->

    <!-- Original, born visible.  The obvious situation, we render the element as part of the page,
    adding identifying information.  The template sets the "b-original" flag to true by default, for
    this reason.  Children of the element are incorporated (through the modal body templates) as
    originals (visible and/or hidden) by passing along the "b-original" flag. -->

    <!-- Original, born hidden.  The element knows if it should be hidden on the page in an embedded
    knowl via the modal "is-hidden" template.  So a link is written on the page, and the main content
    is written onto the page as a hidden, embedded knowl.  The "b-original" flag (set to true) is
    passed through to templates for the children. -->

    <!-- Duplicates.  Duplicated versions, sans identification, are created by an extra, specialized,
    traversal of the entire document tree with the "make-efficient-knowl" templates  When an element
    is first considered as a cross-reference target the infrastructure for an external file is
    constructed and the modal "body" template of the element is called with the "b-original" flag
    set to false.  The content of the knowl should have an overall heading, explaining what it is,
    since it is a target of the cross-reference.  Now the body template will pass along the
    "b-original" flag set to false, indicating the production mode should be duplication.  -->

    <!-- Child elements born visible will be written into knowl files without identification.  -->

    <!-- The upshot is that the main pages have visible content and hidden, embedded content
    (knowls) with full identification as original canonical versions.  Cross-references open external
    file knowls.  None of the knowl files contain any identification, so these identifiers remain
    unique in their appearances as part of the main pages. -->

    <!-- This process is controlled by the boolean "b-original" parameter, which needs to be
    laboriously passed down and through templates, including containers like "sidebyside."  The XSLT
    2.0 tunnel parameter would be a huge advantage here.  The parameter "block-type" can take on the
    values: 'visible', 'hidden', 'xref'.  The three situations above can be identified with these
    parameters.  The block-type parameter is also used to aid in placement of identification.  For
    example, an element born visible will have an HTML id on its outermost element, such as an
    "article".  But as a born-hidden knowl, we put the id onto the visible link text instead, even if
    the same outermost element is employed for the hidden content.  Also, the block-type parameter is
    tunneled down to the Sage cells so they can be constructed properly when inside of knowls. -->

    <!-- The relevant templates controlling production of a block, and their use, are: -->

    <!-- (1) "is-hidden":  mandatory, value is 'true' or 'false' (could move to a boolean), controls
    visible or hidden property, so usd in a variety of situations to control flow.  Often fixed, but
    also responds to options. (As boolean: do conditionals in global text variable, then check value
    in "select" of new global boolean variable.) -->

    <!-- (2) "body-element", "body-css-class": useful for general production, but sometimes its
    employment leads to requiring exceptional templates (eg display math).  The outermost HTML
    element of a block.  Sometimes it gets an ID, sometimes not, which is its main purpose.  Employed
    in "body" templates (see below).  The "body-element" should always be a block element, since it
    will be the outer-level element for knowl content, which will (always) have blocks as content. -->

    <!-- (3) "heading-birth": produces HTML immediately interior to the "body-element", for visible
    blocks, in both the original and duplication processes.  Similarly, it is the link-text of a
    knowl for a block that is hidden (again in original or duplication modes).  Employed in "body"
    templates. -->

    <!-- (4) "heading-xref-knowl": when a knowl is a target of a cross-reference, sometimes a better
    heading is necessary to help identify it.  For example, a cross-refernce to a list item can be
    improved by providing the number of the item in a heading. -->

    <!-- (5) "body": main template to produce the HTML "body" portion of a knowl, or the content
    displayed on a page.  Reacts to four modes: 'visible' (original or duplicate), 'hidden', or
    'xref'. -->

    <!-- (6) TODO: "wrapped-content" called by "body" to separate code. -->

    <xsl:template
        match="&REMARK-LIKE;|&COMPUTATION-LIKE;|&DEFINITION-LIKE;|&ASIDE-LIKE;|poem|&FIGURE-LIKE;|assemblage|blockquote|paragraphs|&GOAL-LIKE;|&OPENPROBLEM-LIKE;|&EXAMPLE-LIKE;|subexercises|exercisegroup|exercise|&PROJECT-LIKE;|task|&SOLUTION-LIKE;|&DISCUSSION-LIKE;|&THEOREM-LIKE;|&AXIOM-LIKE;|&PROOF-LIKE;|case|contributor|biblio|biblio/note|interactive/instructions|gi|p|li|me|men|md|mdn|fragment">
        <xsl:param name="b-original" select="true()" />
    ...
    <xsl:template match="sidebyside/figure|sidebyside/table|sidebyside/listing|sidebyside/list"
        mode="is-hidden">
        <xsl:value-of select="false()" />
    </xsl:template>
    ...
    <xsl:template match="&FIGURE-LIKE;" mode="wrapped-content">
        ...
        <xsl:variable name="b-place-title-below"
            select="($fig-placement = 'subnumber') or ($fig-placement = 'panel')" />
        <xsl:choose>
    ...
    <xsl:template match="assemblage" mode="heading-birth">
        <xsl:apply-templates select="." mode="assemb-title" />
    </xsl:template>
    ...
    <xsl:template match="assemblage" mode="wrapped-content">
        ...
        <!-- Coordinate with schema, since we enforce it here -->
        <xsl:apply-templates
            select="p|blockquote|pre|image|video|program|console|tabular|sidebyside|sbsgroup">
            <xsl:with-param name="b-original" select="$b-original" />
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="exercise" mode="wrapped-content">
        ...
        <xsl:choose>
            ...
            <xsl:when test="task">
                ...
                <xsl:variable name="b-tabbed-tasks"
                    select="
    ...
    <xsl:template match="task" mode="wrapped-content">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="divisional"
                    select="$exercise-container and $exercise-container/ancestor::exercises" />
                <xsl:variable name="worksheet"
                    select="$exercise-container and $exercise-container/ancestor::worksheet" />
                <xsl:variable name="reading"
                    select="$exercise-container and $exercise-container/ancestor::reading-questions" />
                <xsl:variable name="inline"
                    select="$exercise-container and not($divisional or $worksheet or $reading)" />
    ...
    <xsl:template
        match="exercise|&PROJECT-LIKE;|task|&EXAMPLE-LIKE;|webwork-reps/static|webwork-reps/static/task|webwork-reps/static/stage"
        mode="exercise-components">
        <xsl:param name="b-original" />
        <xsl:param name="block-type" />
        <xsl:param name="b-has-statement" />
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="(@exercise-interactive = 'truefalse') or
    ...
    <xsl:template match="*" mode="solutions-div">
        ...
        <xsl:if
            ...
            <xsl:choose>
                <xsl:when
                    test="($knowl-example-solution = 'no') and ancestor-or-self::*[&EXAMPLE-FILTER;]">
    ...
    <xsl:template match="p[ol|ul|dl|me|men|md|mdn|cd]" mode="body">
        ...
        <xsl:variable name="initial"
            select="$displays[1]/preceding-sibling::*|$displays[1]/preceding-sibling::text()" />
        ...
        <div class="para logical">
            ...
            <xsl:for-each select="$displays">
                ...
                <xsl:variable name="rightward"
                    select="following-sibling::*|following-sibling::text()" />
                <xsl:variable name="next-display"
                    select="following-sibling::*[self::ul or self::ol or self::dl or self::me or self::men or self::md or self::mdn or self::cd][1]" />
                <xsl:choose>
                    <xsl:when test="$next-display">
                        <xsl:variable name="leftward"
                            select="$next-display/preceding-sibling::*|$next-display/preceding-sibling::text()" />
                        <!-- device below forms set intersection -->
                        <xsl:variable name="common"
                            select="$rightward[count(. | $leftward) = count($leftward)]" />
            ...
            <xsl:otherwise>
                <xsl:element name="li">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when
                            test="p|blockquote|pre|image|video|program|console|tabular|&FIGURE-LIKE;|&ASIDE-LIKE;|sidebyside|sbsgroup|sage">
                            <xsl:apply-templates>
                                <xsl:with-param name="b-original" select="$b-original" />
                            </xsl:apply-templates>
                        </xsl:when>
    ...
    <xsl:template
        match="introduction[not(parent::*[&STRUCTURAL-FILTER;])]|conclusion[not(parent::*[&STRUCTURAL-FILTER;])]">
        <xsl:param name="b-original" select="true()" />
    ...
    <xsl:template match="image[latex-image]|image[pf:prefigure]" mode="image-inclusion">
        ...
        <xsl:choose>
            <xsl:when
                test="$b-portable-html and (latex-image|pf:prefigure[not(pf:diagram/pf:annotations)])">
    ...
    <xsl:template match="image[asymptote]" mode="image-inclusion">
        ...
        <xsl:variable name="width">
            <xsl:choose>
                ...
                <!-- failure -->
                <xsl:otherwise>
                    <xsl:message>PTX:ERROR: the Asymptote diagram produced in "<xsl:value-of
                            select="$image-xml" />" needs to be available relative to the primary
                        source file, or if available it is perhaps ill-formed and its width cannot
                        be determined (which you might report as a bug). We might be able to proceed
                        as if the diagram is square, but results can be unpredictable.</xsl:message>
        ...
        <xsl:variable name="height">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:message>PTX:ERROR: the Asymptote diagram produced in "<xsl:value-of
                            select="$image-xml" />" needs to be available relative to the primary
                        source file, or if available it is perhaps ill-formed and its height cannot
                        be determined (which you might report as a bug). We might be able to proceed
                        as if the diagram is square, but results can be unpredictable.</xsl:message>
                    <!-- reasonable guess at points/pixels -->
                    <xsl:text>400</xsl:text>
    ...
    <xsl:template name="relative-width">
        ...
        <xsl:value-of
            select="(100 * substring-before($width, '%')) div (100 - substring-before($left-margin, '%') - substring-before($right-margin, '%'))" />
        <xsl:text>%</xsl:text>
    </xsl:template>
                ...
                <xsl:when test="$b-host-runestone and @youtube">
                    ...
                    <xsl:apply-templates select="." mode="runestone-youtube-embed">
                        <xsl:with-param name="width"
                            select="($layout/width * $design-width) div 100" />
                        <xsl:with-param name="height"
                            select="($layout/height * $design-width) div 100" />
                    </xsl:apply-templates>
                </xsl:when>
                <xsl:otherwise>
                ...
                <div>
                    ...
                    <xsl:attribute name="style">
                        ...
                        <xsl:if test="self::video">
                            ...
                            <xsl:variable name="height-percent"
                                select="100 * ($layout/height div $layout/width)" />
                            <xsl:text>padding-top: </xsl:text>
    ...
    <xsl:template match="*" mode="standalone-page">
        ...
        <exsl:document href="{$filename}" method="html" indent="yes" encoding="UTF-8">
            <xsl:call-template name="jekyll-frontmatter" />
            <xsl:call-template name="converter-blurb-html-no-date" />
            <html>
                <xsl:call-template name="language-attributes" />
                            ...
                            <div class="title-container">
                                <h1 class="heading">
                                    ...
                                    <a href="{$root-filename}">
                                        <xsl:variable name="b-has-subtitle"
                                            select="boolean($document-root/subtitle)" />
                                        <span class="title">
    ...
    <xsl:template name="generic-preview-svg">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 14 96 68"
            style="cursor:pointer; position: absolute; top: 0; left: 0; width: 100%; height: 100%;"
            preserveAspectRatio="none">
            <path fill="#e62117"
                d="M94.98,28.84c0,0-0.94-6.6-3.81-9.5c-3.64-3.81-7.72-3.83-9.59-4.05c-13.4-0.97-33.52-0.85-33.52-0.85s-20.12-0.12-33.52,0.85c-1.87,0.22-5.95,0.24-9.59,4.05c-2.87,2.9-3.81,9.5-3.81,9.5S0.18,36.58,0,44.33v7.26c0.18,7.75,1.14,15.49,1.14,15.49s0.93,6.6,3.81,9.5c3.64,3.81,8.43,3.69,10.56,4.09c7.53,0.72,31.7,0.89,32.54,0.9c0.01,0,20.14,0.03,33.54-0.94c1.87-0.22,5.95-0.24,9.59-4.05c2.87-2.9,3.81-9.5,3.81-9.5s0.96-7.75,1.02-15.49v-7.26C95.94,36.58,94.98,28.84,94.98,28.84z M38.28,61.41v-27l25.74,13.5L38.28,61.41z" />
        </svg>
    </xsl:template>
    ...
    <xsl:variable name="watermark-svg">
        <svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="600" width="600">
            <text x="50%" y="50%" text-anchor="middle" transform="rotate(-45,300,300)"
                fill="rgb(204,204,204)"
                style="font-family:sans-serif; font-size:{5*$watermark-scale}cm;">
                <xsl:value-of select="$watermark-text" />
            </text>
        </svg>
    </xsl:variable>
    ...
    <xsl:template match="row">
        ...
        <xsl:element name="tr">
            ...
            <xsl:call-template name="row-cells">
                ...
                <xsl:with-param name="left-col" select="ancestor::tabular/col[1]" />  <!-- possibly
                empty -->
            </xsl:call-template>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template name="row-cells">
        ...
        <xsl:variable name="right-col"
            select="($left-col/self::*|$left-col/following-sibling::col)[position()=$column-span]" />
        ...
        <xsl:variable name="next-col" select="$right-col/following-sibling::col[1]" /> <!-- possibly
        empty -->

        <!-- Check if row-headers are requested -->
        <xsl:variable name="b-row-headers"
            select="boolean($the-cell/parent::row/parent::tabular[@row-headers = 'yes'])" />
        <!-- And if we are at the first cell -->
        <xsl:variable name="b-row-header"
            select="$b-row-headers and not($the-cell/preceding-sibling::cell)" />
                    ...
                    <xsl:attribute name="style">
                        ...
                        <xsl:choose>
                            <xsl:when test="$left-col/@width">
                                ...
                                <xsl:value-of
                                    select="$design-width * substring-before($width, '%') div 100 * substring-before($ambient-relative-width, '%') div 100" />
                            ...
                            <xsl:otherwise>
                                <xsl:value-of
                                    select="$design-width * 0.2 * substring-before($ambient-relative-width, '%') div 100" />
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:attribute>
                </xsl:if>
    ...
    <xsl:template match="*" mode="xref-link">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="ancestor::webwork-reps|ancestor::title|ancestor::shorttitle|ancestor::subtitle">
                <xsl:copy-of select="$content" />
            </xsl:when>
    ...
    <xsl:template match="unit|per">
        ...
        <xsl:if
            test="(self::unit and preceding-sibling::unit) or (self::per and preceding-sibling::per)">
    ...
    <xsl:include href="./core/html-symbols.xsl" />
    ...
    <xsl:template name="insert-symbol">
        ...
        <xsl:if test="$entity-name=''">
            <xsl:message>PTX:ERROR: the symbol name <xsl:value-of select="$name" /> is not a known
                symbol. It will not render correctly.</xsl:message>
        </xsl:if>

        <!-- Could also just use $name here if assume browser supports ligatures. (Just about all
        do)  -->
    ...
    <xsl:template match="program" mode="code-inclusion">
        ...
        <xsl:variable name="b-has-code"
            select="not(normalize-space(code) = '') or preamble[@visible = 'yes'] or postamble[@visible = 'yes']" />
            ...
            <code>
                ...
                <xsl:variable name="program-text">
                    <xsl:if test="preamble[not(@visible = 'no')]">
                        ...
                        <!-- code section MUST end with newline, which author may or may not have
                        included      -->
                        <!-- so remove up to one newline and trailing space from end of code then
                        add a newline -->
    ...
    <xsl:template match="interactive[@geogebra]" mode="iframe-interactive">
        ...
        <!-- ctl = click to launch                 -->
        <iframe
            src="https://www.geogebra.org/material/iframe/id/{@geogebra}/width/{$ggbMaterialWidth}/height/{$ggbMaterialHeight}/border/888888/smb/false/stb/{$ggbToolBar}/stbh/{$ggbToolBar}/ai/{$ggbAlgebraInput}/asb/false/sri/{$ggbResetIcon}/rc/false/ld/false/sdz/{$ggbShiftDragZoom}/ctl/false">
            <xsl:apply-templates select="." mode="html-id-attribute" />
            <xsl:apply-templates select="." mode="size-pixels-attributes" />
            <xsl:apply-templates select="." mode="iframe-dark-mode-attribute" />
        </iframe>
        ...
        <xsl:variable name="cp3d-endpoint">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:message>PTX:ERROR: @variant="<xsl:value-of select="@variant" />" is not
                        recognized for a CalcPlot3D &lt;interactive&gt;</xsl:message>
                    <xsl:apply-templates select="." mode="location-report" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template match="interactive[@platform]" mode="create-iframe-page">
        ...
        <exsl:document href="{$if-filename}" method="html" indent="yes" encoding="UTF-8"
            doctype-system="about:legacy-compat">
            <xsl:call-template name="converter-blurb-html-no-date" />
            <html>
                <xsl:call-template name="language-attributes" />
    ...
    <xsl:template match="interactive[@desmos|@geogebra|@calcplot3d|@circuitjs|@iframe]"
    ...
    <xsl:template match="interactive[@platform = 'jsxgraph']" mode="header-libraries">
        <link rel="stylesheet" type="text/css"
            href="https://cdnjs.cloudflare.com/ajax/libs/jsxgraph/1.8.0/jsxgraph.css" />
        <script type="text/javascript"
            src="https://cdnjs.cloudflare.com/ajax/libs/jsxgraph/1.8.0/jsxgraphcore.js"></script>
    </xsl:template>
    ...
    <xsl:template match="interactive[@platform = 'doenetml']" mode="header-libraries">
        <link rel="stylesheet" type="text/css"
            href="https://cdn.jsdelivr.net/npm/doenet-standalone-test@0.6.0/dist/style.css" />
        <script onload="onLoad()" type="module"
            src="https://cdn.jsdelivr.net/npm/doenet-standalone-test@0.6.0/dist/doenet-standalone.min.js"></script>
        <script>
    ...
    <xsl:template match="slate[@surface='geogebra']">
        ...
        <script>
            ...
            <xsl:if test="normalize-space(text())">
                ...
                <!-- call a function named 'listeners()' in attached .js file to communicate with
                other divs -->
            ...
            <xsl:choose>
                ...
                <xsl:when test="@geogebra">
                    <xsl:message>PTX Warning: "geogebra" attribute on "slate" element is deprecated;
                        use "material" attribute</xsl:message>
                    <xsl:text>material_id:"</xsl:text>
    ...
    <xsl:template name="webwork-js">
        <xsl:if test="$b-has-webwork-reps">
            <xsl:choose>
                <xsl:when test="$webwork-reps-version = 1">
                    <script
                        src="{$webwork-domain}/webwork2_files/js/vendor/iframe-resizer/js/iframeResizer.min.js"></script>
                </xsl:when>
                <xsl:when test="$webwork-reps-version = 2">
                    <script
                        src="/assets/js/pretext/pretext-webwork/2.{$webwork-minor-version}/pretext-webwork.js"></script>
                    <script
                        src="{$webwork-domain}/webwork2_files/node_modules/iframe-resizer/js/iframeResizer.min.js"></script>
                </xsl:when>
            </xsl:choose>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="lti-iframe-resizer">
        <script src="/assets/js/pretext/lti_iframe_resizer.js"></script>
    </xsl:template>
    ...
    <xsl:template match="webwork-reps">
        ...
        <xsl:variable name="b-has-hint"
            select="(ancestor::*[&PROJECT-FILTER;] and $b-has-project-hint) or
        ...
        <xsl:variable name="b-has-answer"
            select="(ancestor::*[&PROJECT-FILTER;] and $b-has-project-answer) or
        ...
        <xsl:variable name="b-has-solution"
            select="(ancestor::*[&PROJECT-FILTER;] and $b-has-project-solution) or
        ...
        <xsl:variable name="b-static"
            select="(ancestor::*[&PROJECT-FILTER;] and $b-webwork-project-static) or
    ...
    <xsl:template match="webwork-reps" mode="webwork-randomize-buttons">
        <div class="WW-randomize-buttons">
            <button class="WW-randomize" type="button" onclick="WWiframeReseed('{@ww-id}')">
                Randomize</button>
            <button class="WW-randomize" type="button"
                onclick="WWiframeReseed('{@ww-id}',{static/@seed})">Reset</button>
        </div>
    </xsl:template>
    ...
    <xsl:template match="myopenmath">
        ...
        <div id="mom{$an-id}wrap" style="width:100%;overflow:visible;position:relative">
            ...
            <iframe id="mom{$an-id}"
                style="position:absolute;z-index:1;object-fit: contain; width: 100%"
    ...
    <xsl:template match="*" mode="file-wrap">
        ...
        <xsl:if test="$debug.editable = 'yes'">
            ...
            <exsl:document href="{$the-source-filename}" method="xml" omit-xml-declaration="no"
                indent="yes" encoding="UTF-8">
                <xsl:copy-of select="." />
            </exsl:document>
        </xsl:if>

        <!-- <exsl:document href="{$the-filename}" method="html" indent="yes" encoding="UTF-8"
        doctype-system="about:legacy-compat"> -->
        <exsl:document href="{$the-filename}" method="html" indent="yes" encoding="UTF-8">
            <xsl:call-template name="jekyll-frontmatter" />
            <xsl:call-template name="converter-blurb-html-no-date" />
            <html>
                ...
                <body>
                    ...
                    <div id="ptx-page-footer" class="ptx-page-footer">
                        ...
                        <xsl:call-template name="cpgem-link" />
                    </div>
    ...
    <xsl:template name="canonical-link">
        ...
        <xsl:variable name="site-root">
            <xsl:value-of
                select="concat('https://runestone.academy/ns/books/published/', $document-id, '/')" />
        </xsl:variable>
        ...
        <xsl:choose>
            <xsl:when test="$intermediate='true'">
                ...
                <xsl:if test="$structural='false'">
                    <xsl:message>PTX:ERROR: descending into first node of an intermediate page (<xsl:value-of
                            select="local-name($first-structural-child)" />) that is non-structural;
                        maybe your source has incorrect structure</xsl:message>
                    <xsl:apply-templates select="." mode="location-report" />
                </xsl:if>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="*" mode="previous-descent-url">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="$structural='false'">
                    <xsl:message>PTX:ERROR: descending into last node of an intermediate page (<xsl:value-of
                            select="local-name($last-structural-child)" />) that is non-structural</xsl:message>
                </xsl:if>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="calculator-toggle">
        <button id="calculator-toggle" class="calculator-toggle button" title="Show calculator"
            aria-expanded="false" aria-controls="calculator-container">
            <xsl:call-template name="insert-symbol">
    ...
    <xsl:template name="mode-toggle">
        <button type="button" id="mode-toggle" class="light-dark-button button" title="Toggle Mode">
            <xsl:call-template name="insert-symbol">
                <xsl:with-param name="name" select="'brightness_medium'" />
            </xsl:call-template>
            <span class="name">Toggle Mode</span>
        </button>
    </xsl:template>

    <!-- <button type="button" class="btn btn-link nav-link" aria-label="Switch Mode"
    id="mode-toggle">
    ...
    <xsl:template match="*" mode="primary-navigation-other-controls">
        <span class="nav-other-controls">
            ...
            <!-- <xsl:call-template name="mode-toggle" /> -->
        </span>
    </xsl:template>
    ...
    <xsl:template name="calculator">
        <xsl:if test="contains($html-calculator,'geogebra')">
            ...
            <script>
                ...
                <!-- But the customization should be different depending on classic, graphing,
                geometry, or 3d. -->
                ...
                <!-- The last parameters have to do with scaling. This combination allows the
                330x600 applet -->
                <!-- to scale up or down to the width of the contining div with class
    ...
    <!-- NB no "book", "article" -->
    <xsl:template
        match="frontmatter|abstract|frontmatter/colophon|biography|dedication|acknowledgement|preface|contributors|part|chapter|section|subsection|subsubsection|exercises|solutions|reading-questions|references|glossary|worksheet|backmatter|appendix|index|backmatter/colophon"
        mode="toc-item">
        <li>
            <xsl:apply-templates select="." mode="toc-item-properties" />
            <!-- Recurse into children divisions (if any)-->
            <xsl:variable name="child-list"
                select="frontmatter|abstract|frontmatter/colophon|biography|dedication|acknowledgement|preface|contributors|part|chapter|section|subsection|subsubsection|exercises|solutions|reading-questions|references|glossary|worksheet|backmatter|appendix|index|backmatter/colophon" />
    ...
    <xsl:template match="ul|li" mode="customized-toc-item">
        ...
        <xsl:variable name="is-ancestor"
            select="not(ancestor::ul) or count($this-page-ancestors|.) = count($this-page-ancestors)" />
        ...
        <xsl:choose>
            ...
            <xsl:when test="$is-ancestor or $is-page">
                ...
                <xsl:copy>
                    ...
                    <xsl:apply-templates select="@*[name() != 'uid' and name() != 'class']|*|text()"
                        mode="customized-toc-item">
                        <xsl:with-param name="this-page" select="$this-page" />
                        <xsl:with-param name="this-page-ancestors" select="$this-page-ancestors" />
                    </xsl:apply-templates>
                </xsl:copy>
            ...
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:apply-templates select="@*[name() != 'uid']|*|text()"
                        mode="customized-toc-item">
                        <xsl:with-param name="this-page" select="$this-page" />
                        <xsl:with-param name="this-page-ancestors" select="$this-page-ancestors" />
                    </xsl:apply-templates>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <division type="mainmatter" number="" id="mainmatter">
                        ...
                        <xsl:apply-templates select="*[not(self::frontmatter or self::backmatter)]"
                            mode="toc-item-list" />
                    </division>
    ...
    <xsl:template
        match="frontmatter|frontmatter/colophon|biography|dedication|acknowledgement|preface|contributors|part|chapter|section|subsection|subsubsection|exercises|solutions|reading-questions|references|glossary|worksheet|backmatter|appendix|index|backmatter/colophon"
        mode="toc-item-list">
        <division>
            <xsl:apply-templates select="." mode="doc-manifest-division-attributes" />
            <!-- Recurse into children divisions (if any)                 -->
    ...
    <xsl:template
        match="&DEFINITION-LIKE;|&THEOREM-LIKE;|&PROOF-LIKE;|&AXIOM-LIKE;|&REMARK-LIKE;|&COMPUTATION-LIKE;|&OPENPROBLEM-LIKE;|&EXAMPLE-LIKE;|&PROJECT-LIKE;|&GOAL-LIKE;|&FIGURE-LIKE;|exercise|p[descendant::term]"
        mode="search-block-docs-textbook">
    ...
    <xsl:template
        match="&STRUCTURAL;|&DEFINITION-LIKE;|&THEOREM-LIKE;|&PROOF-LIKE;|&AXIOM-LIKE;|&REMARK-LIKE;|&COMPUTATION-LIKE;|&OPENPROBLEM-LIKE;|&EXAMPLE-LIKE;|&PROJECT-LIKE;|&GOAL-LIKE;|&FIGURE-LIKE;|exercise|p|blockquote|pre"
        mode="search-block-docs-reference">
        <!-- build a search document and dead-end -->
        <xsl:apply-templates select="." mode="search-document">
            <xsl:with-param name="level" select="'2'" />
        </xsl:apply-templates>
...
-->
            ...
            <div class="logo">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" height="100%" viewBox="338 3000 8772 6866"
                    ...
                    <g style="stroke-width:.025in; stroke:currentColor; fill:none">
                        <polyline points="472,3590 472,9732 "
                            style="stroke-width:174; stroke-linejoin:miter; stroke-linecap:round; " />
                        <path style="stroke-width:126;stroke-linecap:butt;"
                            d="M 4724,9448 A 4660 4660  0  0  1  8598  9259" />
                        <path style="stroke-width:174;stroke-linecap:butt;"
                            d="M 4488,9685 A 4228 4228  0  0  0   472  9732" />
                        <path style="stroke-width:126;stroke-linecap:butt;"
                            d="M 4724,3590 A 4241 4241  0  0  1  8598  3496" />
                        <path style="stroke-width:126;stroke-linecap:round;"
                            d="M 850,3496  A 4241 4241  0  0  1  4724  3590" />
                        <path style="stroke-width:126;stroke-linecap:round;"
                            d="M 850,9259  A 4507 4507  0  0  1  4724  9448" />
                        <polyline points="5385,4299 4062,8125"
                            style="stroke-width:300; stroke-linejoin:miter; stroke-linecap:round;" />
                        <polyline points="8598,3496 8598,9259"
                            style="stroke-width:126; stroke-linejoin:miter; stroke-linecap:round;" />
                        <polyline points="850,3496 850,9259"
                            style="stroke-width:126; stroke-linejoin:miter; stroke-linecap:round;" />
                        <polyline points="4960,9685 4488,9685"
                            style="stroke-width:174; stroke-linejoin:miter; stroke-linecap:round;" />
                        <polyline points="3070,4582 1889,6141 3070,7700"
                            style="stroke-width:300; stroke-linejoin:miter; stroke-linecap:round;" />
                        <polyline points="6418,4582 7600,6141 6418,7700"
                            style="stroke-width:300; stroke-linejoin:miter; stroke-linecap:round;" />
                        <polyline points="8976,3590 8976,9732"
                            style="stroke-width:174; stroke-linejoin:miter; stroke-linecap:round;" />
                        <path style="stroke-width:174;stroke-linecap:butt;"
    ...
    <xsl:template name="cpgem-link">
        <a class="cpgem-link" href="https://cpgem.github.io" title="CPGEM">
            <img class="logo" src="https://cpgem.github.io/assets/img/favicons/favicon.png"
                alt="CPGEM logo" />
        </a>
    </xsl:template>
    ...
    <xsl:template name="runestone-ethical-ads">
        <xsl:if test="$b-host-runestone">
            ...
            <div style="width: 100%">
                <div data-ea-publisher="runestoneacademy" data-ea-type="image"
                        ...
                        <map key="tex">
                            ...
                            <map key="packages">
                                <array key="[+]">
                                    ...
                                    <xsl:if
                                        test="not(contains($latex-packages-mathjax, '\require{extpfeil}'))">
    ...
    <xsl:template match="*" mode="sagecell">
        ...
        <xsl:if test="$b-has-sage">
            ...
            <xsl:if
                test=".//sage[not(@type) and (not(@language) or @language='sage') and not(@auto-evaluate = 'yes')]">
            ...
            <xsl:if
                test=".//sage[not(@type) and (not(@language) or @language='sage') and (@auto-evaluate = 'yes')]">
                <xsl:call-template name="makesagecell">
                    <xsl:with-param name="language-attribute">
                        <xsl:text>sage</xsl:text>
                    </xsl:with-param>
    ...
    <xsl:template name="syntax-highlight">
        <xsl:if test="$b-has-program and not($b-debug-react)">
            <script
                src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.26.0/components/prism-core.min.js"></script>
            <script
                src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.26.0/plugins/autoloader/prism-autoloader.min.js"></script>
            <script
                src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.26.0/plugins/line-numbers/prism-line-numbers.min.js"
                integrity="sha512-dubtf8xMHSQlExGRQ5R7toxHLgSDZ0K7AunqPWHXmJQ8XyVIG19S1T95gBxlAeGOK02P4Da2RTnQz0Za0H0ebQ=="
                crossorigin="anonymous" referrerpolicy="no-referrer"></script>
            <script
                src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.26.0/plugins/line-highlight/prism-line-highlight.min.js"
            ...
            <xsl:if test="contains($html-theme-name, '-legacy')">
                ...
                <link href="https://cdnjs.cloudflare.com/ajax/libs/prism/1.26.0/themes/prism.css"
                    rel="stylesheet" />
                ...
                <link rel="stylesheet"
                    href="https://cdnjs.cloudflare.com/ajax/libs/prism/1.26.0/plugins/line-highlight/prism-line-highlight.min.css"
    ...
    <xsl:template name="native-search-box-js">
        <xsl:if test="$has-native-search and not($b-debug-react)">
            ...
            <script src="https://cdn.jsdelivr.net/npm/lunr-languages@1.14.0/lunr.fr.js"></script>
            ...
            <script src="/assets/js/pretext/pretext_search.js" />
        ...
        <xsl:if test="$has-native-search">
            <div class="searchbox">
                <div class="searchwidget">
                    <button id="searchbutton" class="searchbutton button" type="button"
                        title="Search book">
                        <xsl:call-template name="insert-symbol">
    ...
    <xsl:template name="native-search-results">
        <xsl:if test="$has-native-search">
            <div id="searchresultsplaceholder" class="searchresultsplaceholder"
                style="display: none">
                <div class="search-results-controls">
                    <input aria-label="Search term" id="ptxsearch" class="ptxsearch" type="text"
                        name="terms" placeholder="Search term" />
                    <button title="Close search" id="closesearchresults" class="closesearchresults">
    ...
    <xsl:template match="*" mode="knowl">
        <xsl:if test="not($b-debug-react)">
            <script src="/assets/js/pretext/lib/knowl.js"></script>
    ...
    <xsl:template name="pretext-js">
        <xsl:choose>
            <xsl:when test="not($b-debug-react)">
                ...
                <script src="/assets/js/pretext/lib/jquery.min.js"></script>
                <script src="/assets/js/pretext/lib/jquery.sticky.js"></script>
                <script src="/assets/js/pretext/lib/jquery.espy.min.js"></script>
                <script src="/assets/js/pretext/pretext.js"></script>
                <script src="/assets/js/pretext/pretext_add_on.js?x=1"></script>
                <script src="/assets/js/pretext/user_preferences.js"></script>
                <!-- script cpgem -->
                <!-- <script src="/assets/js/dist/theme.min.js"></script>
            <script defer="" src="/assets/lib/simple-jekyll-search/simple-jekyll-search.min.js"></script>
            <script defer=""
                src="/assets/lib/loading-attribute-polyfill/loading-attribute-polyfill.umd.min.js"></script>
    ...
    <xsl:template name="fonts">
        ...
        <xsl:if test="contains($html-theme-name, '-legacy')">
            ...
            <link
                ...
                rel="stylesheet" />
            <!-- NB: not loading (binary) italic axis for variable fonts, tests seem to indicate
            this is OK -->
            <link href="https://fonts.googleapis.com/css2?family=Inconsolata:wght@400;700&amp;"
                rel="stylesheet" />
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="jsxgraph">
        <xsl:if test="$b-has-jsxgraph">
            <link rel="stylesheet" type="text/css"
                href="https://cdnjs.cloudflare.com/ajax/libs/jsxgraph/0.99.6/jsxgraph.css" />
    ...
    <xsl:template name="mermaid-header">
        <xsl:if test="$b-has-mermaid">
            <script type="module"> import mermaid from
                'https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs'; let theme = '<xsl:value-of
                    select="$mermaid-theme" />'; if (isDarkMode()) theme = 'dark';
                mermaid.initialize({ securityLevel: 'loose', theme: theme, }); </script>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="diagcess-header">
        <xsl:if test="$b-has-prefigure-annotations">
            <script src="/assets/js/pretext/diagcess/diagcess.js"></script>
        </xsl:if>
    </xsl:template>
            ...
            <noscript>
                <div class="statcounter">
                    <a title="web analytics" href="https://statcounter.com/" target="_blank">
                        <img class="statcounter" src="{$noscript_url}" alt="web analytics" />
                    </a>
                </div>
            </noscript>
    ...
    <xsl:template match="*" mode="project-info">
        <xsl:text>project: </xsl:text>
        <xsl:apply-templates select="$document-root" mode="string-id" />
        <xsl:text>&#xa;project_id: </xsl:text>
        <xsl:apply-templates select="$document-root" mode="visible-id" />
        <xsl:text>&#xa;project_title: "</xsl:text>
        <xsl:apply-templates select="$document-root" mode="title-plain" />
        <xsl:if test="$document-root/subtitle">
            <xsl:text>"&#xa;project_subtitle: "</xsl:text>
            <xsl:apply-templates select="$document-root" mode="subtitle" />
        </xsl:if>
        <xsl:text>"&#xa;project_url: "</xsl:text>
    ...
    <xsl:template name="get-field-value">
        ...
        <xsl:value-of select="str:split($pairs[contains(., concat($field, ':'))], ': ')[2]" />
    </xsl:template>

    <!-- Template pour récupérer les variables structurées -->
    <xsl:template match="chapter" mode="title-formated">

        <xsl:variable name="title-plain">
            <xsl:apply-templates select="." mode="title-plain" />
        </xsl:variable>
        <xsl:variable name="pairs" select="str:split($title-plain, ', ')" />


        <xsl:variable name="concours">
            <xsl:call-template name="get-field-value">
                <xsl:with-param name="pairs" select="$pairs" />
                <xsl:with-param name="field" select="'concours'" />
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="filiere">
            <xsl:call-template name="get-field-value">
                <xsl:with-param name="pairs" select="$pairs" />
                <xsl:with-param name="field" select="'filiere'" />
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="session">
            <xsl:call-template name="get-field-value">
                <xsl:with-param name="pairs" select="$pairs" />
                <xsl:with-param name="field" select="'session'" />
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="epreuve">
            <xsl:call-template name="get-field-value">
                <xsl:with-param name="pairs" select="$pairs" />
                <xsl:with-param name="field" select="'epreuve'" />
            </xsl:call-template>
        </xsl:variable>
    ...
    <xsl:template match="chapter" mode="sujet-meta">
        ...
        <!-- Génération du titre formaté sans balises HTML -->
    ...
    <!-- <xsl:template match="*" mode="sujet-info">
    <xsl:variable name="is-chapter" select="self::chapter" />
    <xsl:variable name="within-chapter" select="ancestor::*[self::chapter]" />
    <xsl:if test="$is-chapter">
        <xsl:text>sujet_id: </xsl:text>
        <xsl:apply-templates select="$is-chapter" mode="visible-id" />
        <xsl:text>&#xa;sujet_meta: {</xsl:text>
        <xsl:apply-templates select="$is-chapter" mode="title-plain" />
        <xsl:text>}&#xa;sujet_url: "</xsl:text>
        <xsl:apply-templates select="$is-chapter" mode="url" />
        <xsl:text>"&#xa;</xsl:text>
    </xsl:if>
    <xsl:if test="$within-chapter">
        <xsl:text>sujet_id: </xsl:text>
        <xsl:apply-templates select="$within-chapter" mode="visible-id" />
        <xsl:text>&#xa;sujet_title: {</xsl:text>
        <xsl:apply-templates select="$within-chapter" mode="title-plain" />
        <xsl:text>}&#xa;sujet_url: "</xsl:text>
        <xsl:apply-templates select="$within-chapter" mode="url" />
        <xsl:text>"&#xa;</xsl:text>
    </xsl:if>
</xsl:template> -->


    <xsl:template name="jekyll-frontmatter">
        <xsl:if test="not($document-id = '')">
            <xsl:variable name="doc-division">
                <xsl:apply-templates select="." mode="string-id" />
            </xsl:variable>
            <xsl:if test="not($doc-division = 'pretext')">
                <xsl:text>---&#xa;</xsl:text>
                <xsl:text>layout: ptx-concours&#xa;</xsl:text>
                <xsl:text>division: </xsl:text>
                <xsl:apply-templates select="." mode="string-id" />
                <xsl:text>&#xa;</xsl:text>
                <xsl:apply-templates select="." mode="project-info" />
                <xsl:if test="self::chapter">
                    <xsl:apply-templates select="." mode="sujet-meta" />
                </xsl:if>
                <xsl:text>---&#xa;&#xa;</xsl:text>
            </xsl:if>
        </xsl:if>
