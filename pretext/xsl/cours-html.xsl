...
<xsl:template name="webwork-js">
    <xsl:if test="$b-has-webwork-reps">
        <xsl:choose>
            ...
            <xsl:when test="$webwork-reps-version = 2">
                <script src="/assets/js/pretext/pretext-webwork/2.{$webwork-minor-version}/pretext-webwork.js"></script>
...
<xsl:template name="cpgem-link">
    <a class="cpgem-link" href="https://cpgem.github.io" title="CPGEM">
        <img class="logo" src="https://cpgem.github.io/assets/img/favicons/favicon.png" alt="CPGEM logo"/>
    </a>
</xsl:template>
...
<xsl:template match="*" mode="chapter-info">
...
<xsl:template name="jekyll-frontmatter">
    <xsl:if test="not($document-id = '')">
        ...
        <xsl:if test="not($doc-division = 'pretext')">
            ...
            <xsl:apply-templates select="." mode="chapter-info" />
            <xsl:text>---&#xa;&#xa;</xsl:text>
        </xsl:if>
    </xsl:if>
 </xsl:template>
