...
function permalinkDescription(elem) {
  ...
  if (
    nodeName == 'P' &&
    elem.parentElement.parentElement.classList.contains('exercisegroup')
  ) {
  ...
  if (isExerciseGroup) {
    headerNode =
      elem.parentElement.parentElement.querySelector(':scope > .heading');
  } else if (nodeName == 'FIGCAPTION') {
    headerNode = elem;
  } else {
    headerNode = elem.querySelector(
      ':scope > .heading, :scope > figcaption, :scope > a > .heading'
    );
  }
  var numberStr = '';
  var numberSep = ' ';
  ...
  if (nodeName == 'P') {
    ...
    if (nodeName == 'ARTICLE' && elem.classList.contains('exercise')) {
      ...
      typeStr = elem.parentElement
        .querySelector(':scope > .autopermalink')
   ...
   same because it toggles twice.  A more elegant solution is welcome */
    ...
    $('a').each(function () {
      if ($(this).parents('.aside-like').length) {
        $(this).attr(
          'addafter',
          '#' + $(this).closest('.aside-like').attr('id')
        );
    ...
    $('body').on(
      'click',
      '.image-box > img:not(.draw_on_me):not(.mag_popup), .sbspanel > img:not(.draw_on_me):not(.mag_popup), figure > img:not(.draw_on_me):not(.mag_popup), figure > div > img:not(.draw_on_me):not(.mag_popup)',
      function () {
...
*/
    ...
    if (document.querySelector('body.standalone')) {
      ...
      items_needing_permalinks = document.querySelectorAll(
        'main section:not(.introduction), main section .para, main section p, main section article, main section > figure.table-like, main section > figure.figure-like > figcaption, main section  .exercisegroup article, main section  .exercisegroup, main section article.exercise, main section .discussion-like,  main section article.paragraphs > figure.table-like, main section article.paragraphs > figure.figure-like, section > details'
      );
      ...
      for (var i = 0; i < items_needing_permalinks.length; i++) {
        try {
          ...
          if (Boolean(this_item.closest('.parsons'))) {
            continue;
          } /* parsons block */
          if (Boolean(this_item.closest('details'))) {
            continue;
          } /* hidden in details */
          if (this_item.parentElement.classList.contains('lines')) {
            continue;
          } /* parsons block */
          if (getComputedStyle(this_item).display == 'inline') {
            continue;
          } /* inline paragraph at start of article, for example*/
          try {
            if (this_item.closest('.hidden-content')) {
              continue;
            }
          } catch {
            // do nothing, because we are just avoiding permalinks on born-hidden knowls
          }
          if (this_item.tagName == 'FIGCAPTION') {
          ...
          if (this_anchor) {
            ...
            this_permalink_container.setAttribute(
              'onclick',
              'copyPermalink(this)'
            );
            this_permalink_container.setAttribute(
            ...
            this_permalink_container.innerHTML =
              '<a href="' +
              this_permalink_url +
              '" title="Copy permalink for ' +
              this_permalink_description +
              '">' +
              permalink_word +
              '</a>';
            ...
            this_item.insertAdjacentElement(
              'beforeend',
              this_permalink_container
            );
          } else {
            /*
...
function WWiframeReseed(iframe, seed) {
  ...
  if (seed === undefined) {
    seed = Number(this_problem.getAttribute('data-seed')) + 80 + 84 + 88;
  }
...
window.addEventListener('load', function (event) {
  ...
  $('body').on('mouseover', '#geogebra-calculator canvas', function () {
    ...
    $('#calculator-container').css(
      'right',
      (calcoffsetR + scrollWidth).toString() + 'px'
    );
    $('#calculator-container').css(
      'bottom',
      (calcoffsetB + scrollWidth).toString() + 'px'
    );
  });
  ...
  if ($('body').attr('id') == 'judson-AATA') {
    ...
    if (typeof eBookConfig !== 'undefined') {
      if (eBookConfig['username']) {
        ...
        newscript.src =
          'https://pretextbook.org/js/' + '0.13' + '/' + 'trails' + '.js';
...
function loadResource(type, file) {
  ...
  if (typeof css_version === 'undefined') {
    css_version = '0.6';
  }
  ...
  if (type == 'css') {
    ...
    newresource.href =
      'https://pretextbook.org/css/' + css_version + '/' + file + '.css';
    newresource.href += '?minivers=' + minivers;
  } else if (type == 'js') {
    newresource.type = 'text/javascript';
    //  newscript.async = true;
    newresource.src =
      'https://pretextbook.org/js/' + js_version + '/' + file + '.js';
...
window.addEventListener('load', function (event) {
  if (false && $('body').attr('id') == 'pretext-SA') {
    ...
    false &&
    window.location.href.includes('soundwriting.pugetsound')
  ) {
  ...
  if (window.location.hash.length) {
    ...
    if (the_anchor.tagName == 'ARTICLE') {
      ...
      if (this_hidden_content) {
        console.log(
          'linked to a hidden knowl with this_hidden_content',
          this_hidden_content
        );
        var the_refid = this_hidden_content.id;
        var this_knowl = document.querySelector(
          '[data-refid="' + the_refid + '"]'
        );
        this_knowl.click();
      }
    }
...
function scaleWorkspaceIn(obj, subobj, scale, tmporfinal) {
  ...
  if (obj != subobj) {
    ...
    if (subobj.classList.contains('workspace')) {
      //we were given one workspace
  ...
  for (var j = 0; j < these_workspaces.length; ++j) {
    ...
    if (tmporfinal == 'final' && scale > 13) {
      ...
      this_work.style.background =
        'linear-gradient( #eef 0px, #eef ' +
        this_proportion_scaledX +
        'px, #eef ' +
        this_proportion_scaledX +
        'px, #99f ' +
        (this_proportion_scaledX + 5) +
        'px, #99f ' +
        (this_proportion_scaledX + 5) +
        'px, #99f 100%)';
    } else {
    ...
    if (tmporfinal == 'final') {
      ...
      var enclosingspacebottom =
        enclosingspace.getBoundingClientRect()['bottom'];
      /* there should be an easier way to do this */
      /* when the enclosing parent has padding, we want to ignore that */
      enclosingspacepadding = parseFloat(
        getComputedStyle(enclosingspace)['padding-bottom'].slice(0, -2)
      );
      enclosingspacebottom = enclosingspacebottom - enclosingspacepadding;
      console.log(
        enclosingspace,
        'enclosingspace padding-bottom',
        getComputedStyle(enclosingspace)['padding-bottom']
      );
      ...
      console.log(
        ...
        enclosingspace,
        'pp',
        enclosingspacebottom,
        'xx',
        lastworkspacebottom,
        'diff',
        enclosingspacebottom - lastworkspacebottom
      );
...
function adjustWorkspace() {
  ...
  if (document.body.classList.contains('a4')) {
    pagelayout = 'a4';
  }
  ...
  for (var i = 0; i < all_pages.length; i++) {
    ...
    if (pagelayout == 'a4') {
      pageheight.push(1100);
    } else {
      pageheight.push(1030);
    }

    this_item = all_pages[i];
    if (i == 0) {
      this_item.classList.add('firstpage');
    }
    /* not else if: could be one-page worksheet */
    if (i == all_pages.length - 1) {
      this_item.classList.add('lastpage');
    }
    console.log(this_item.getBoundingClientRect(), 'ccc', this_item);
    console.log(
      this_item.parentElement.getBoundingClientRect(),
      'ddd',
  ...
  for (var i = 0; i < all_pages.length; i++) {
    ...
    if (this_item.classList.contains('firstpage')) {
      console.log(
        'this_item',
        this_item.getBoundingClientRect(),
        this_item.getBoundingClientRect()['y']
      );
      console.log(
        'this_item parent',
        this_item.parentElement.getBoundingClientRect(),
        this_item.parentElement.getBoundingClientRect()['y']
      );
      pageExtraHeight +=
        pageData['top'] - worksheetData['top']; /* 45 for padding */
    }
...
window.addEventListener('load', function (event) {
  if (document.body.classList.contains('worksheet')) {
    ...
    var born_hidden_knowls = document.querySelectorAll(
      'article > a[data-knowl]'
    );
